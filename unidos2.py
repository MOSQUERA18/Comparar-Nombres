...
def normalizar_nombre(nombre: str) -> str:
    """Normaliza un nombre quitando tildes y mayúsculas/minúsculas, pero sin alterar el orden."""
    return unidecode(nombre.strip().upper())


def comparar_nombres_fuzzy(nombres_pdf, nombres_excel):
    resultados = []
    ...
    for nombre_pdf in nombres_pdf:
        ...
        for nombre_excel in nombres_excel:
            score = fuzz.token_sort_ratio(nombre_pdf_norm, normalizar_nombre(nombre_excel))
            if score > mejor_score:
                mejor_score = score
                mejor_match = nombre_excel  # Siempre guardamos como está en Excel
        ...
        if mejor_score == 100:
            ...
            estado = "NO EXISTE EN EL REPORTE DE INSCRIPCION"

        resultados.append({
            "Nombre PDF": nombre_pdf,
            "Nombre Excel": mejor_match if mejor_match else "—",  # 👈 nunca vacío
            "Porcentaje Similitud": mejor_score,
            "Estado": estado
        })

    # Comparar Excel → PDFs (para "FALTA CERTIFICADO")
    for nombre_excel in nombres_excel:
        nombre_norm = normalizar_nombre(nombre_excel)
        if not any(fuzz.token_sort_ratio(nombre_norm, normalizar_nombre(n)) >= 90 for n in nombres_pdf):
            resultados.append({
                "Nombre PDF": "—",  
                "Nombre Excel": nombre_excel,   # 👈 aquí SI se guarda
...
📁 Los archivos se guardan en la carpeta Descargas"""
    ...
    def exportar_resultados_nombres(self, resultados: List[dict]):
        """Exporta resultados de comparación de nombres a Excel con colores."""
        # Convertir lista de diccionarios a DataFrame
        df_resultados = pd.DataFrame(resultados)

        # Renombrar columnas para mantener consistencia en la salida
        df_resultados = df_resultados.rename(columns={
            "Nombre Excel": "Mejor Coincidencia Excel"
        })

        # Asegurar el orden de columnas
        columnas = ["Nombre PDF", "Mejor Coincidencia Excel", "Porcentaje Similitud", "Estado"]
